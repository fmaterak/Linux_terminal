...
public:
    ...
    class LineRange {
        // std::list has no quick random access, so there is some logic to move around as quickly as possible
    ...
    public:
        ...
        inline LineListIter end() const { return cached_end; }  // Note: end is inclusive
    };
...
private:
    ...
    // std::size_t first_ln, last_ln;

public:
    LineBuffer(int width);

    // inline std::size_t first_line_num() const { return first_ln; }
    // inline std::size_t last_line_num() const { return last_ln; }

    const LineRange& range(std::size_t begin_pos, std::size_t end_pos);

    void read_from(std::istream& stream, std::size_t count);
};
